...
public:
	...
	Qhistogram(int pId,int nprocs,int tw, core::WalkerState<T,U>& _state, string bsf, FILE* log, double _dt) : MPIObservable(pId,nprocs,tw),Observable<T,U>(_state,bsf,log)
	{
		dt = _dt;
		ltime = 0;
		Q.x = 0.0;
